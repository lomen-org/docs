...
| --- | --- | --- | --- |
| Managed wallets (Cesto-managed Privy) | Cesto (Privy) | Cesto, server-side | [`managed.*`](/sdk/managed-wallets) |
...
<Steps>
  ...
  <Step title="Open / close by user">
    Call [`managed.open` / `managed.close` / `managed.rebalance`](/sdk/managed-wallets#opening-closing-rebalancing)
...
<Steps>
  ...
  <Step title="Open / close on Solana">
    ...
    [`managed.open` / `managed.close`](/sdk/managed-wallets#opening-closing-rebalancing) with
    the user reference, investing the **arrived** amount (`netOut`). Read holdings any time
    with [`positions.getPosition`](/sdk/positions#position-by-product-sdk-positions).
  </Step>
</Steps>
