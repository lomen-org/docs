...
<Tabs>
  ...
  <Tab title="BYOW (client-signed)">
    ...
    <ParamField path="product" type="string" required>
      Basket to open — product id or slug.
    </ParamField>
    ...
    <Warning>
      ...
      [`start` with `consent`](#existing-cesto-users): those executions belong to the user
      rather than your key, so a lookup returns `404` however long you poll — read
      [`positions.getHoldings`](/sdk/positions#position-by-product-sdk-positions) instead.
    </Warning>
...
signing, no round trip to the browser for each leg.

It is the same [`open.start`](/sdk/managed-wallets#opening-closing-rebalancing) as the
...
Check first — this path only works for wallets Cesto holds a key for:
...
users take the BYOW flow above. See [Users](/sdk/users).

<Steps>
  <Step title="Create the approval challenge">
    ```ts
    const challenge = await cesto.open.createChallenge({
      wallet: userWalletAddress,
      product: 'stable-genius',
      amount: 100_000_000n,
    });
    // { message, challengeToken, expectedPubkey, expiresAt, productId, … }
    ```
  ...
  <Step title="The user signs the message">
  ...
  <Step title="Start the open">
    ```ts
    const { executionId, status } = await cesto.open.start({
      user: userWalletAddress,
      product: challenge.productId, // the resolved id — skips a second slug lookup
      amount: 100_000_000n,
      consent: { challengeToken: challenge.challengeToken, signature },
    });
    // status: 'QUEUED' | 'SCHEDULED_FOR_MARKET_OPEN'
    ```
  </Step>
  <Step title="Track the result">
    ...
    // holdings.hasPosition flips true once the open completes
    ```
  </Step>
</Steps>
...
<Warning>
  **`positions.getExecution` / `waitForExecution` cannot see these executions.** They are
  ...
  `open.startAndWait` rejects when `consent` is present, instead of polling forever.

  This applies only to the approval flow. The managed and client-signed flows poll normally.
</Warning>
...
[`positions.getHoldings`](/sdk/positions#position-by-product-sdk-positions) — SDK positions
...
| Open while a close is in flight for this wallet + product | `403 PermissionDeniedError`, code `FORBIDDEN_OPERATION` |
