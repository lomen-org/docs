...
title: "Bridging (CCTP)"
icon: "bridge"
description: "Bridge USDC between Base and Solana with Circle CCTP — the user signs once on the source chain, Cesto attests and mints on the destination."
---
...
<Note>
  **Managed wallets skip this step.** When `sourceAddress` is a Cesto-managed Solana wallet
  (provisioned via `users.create` under your API key), Cesto **signs and lands the burn
  server-side** — sponsor pays gas — and `initiate` returns
  `{ status: 'BURN_SUBMITTED', burnTxHash, burn: null }` instead of the unsigned burn. Skip
  `submitBurn` (re-submitting the same hash is safe/idempotent) and go straight to
  `waitForTransfer`. See [Managed wallets — Withdrawing](/sdk/managed-wallets#withdrawing).
</Note>
...
`solana → base` back to the user's EVM wallet.
