...
## Environments (mainnet vs testnet)

The network is chosen by the **chain key**, not by RPC or config:

| Network | Chain keys | Attestation |
| --- | --- | --- |
| Mainnet | `base`, `ethereum`, `solana` | Circle production Iris |
| Testnet | `base-sepolia`, `ethereum-sepolia`, `solana-devnet` | Circle sandbox Iris |

- **Production deployments reject testnet keys** (and vice versa for routes mixing
  networks — e.g. `base → solana-devnet` is a `400`). Both legs of a transfer stay in one
  network.
- Test integrations run against Circle's sandbox attestation automatically — no extra
  config. Get test USDC from [Circle's faucet](https://faucet.circle.com) (per chain).
- RPC overrides (`BRIDGE_RPC_*`) only change *which node* the backend talks to on a given
  chain — they never switch the network.
